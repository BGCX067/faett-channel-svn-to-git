...
// do not create rewrite for admin backend (store_id == 0)
$query = $conn->query(
    'SELECT store_id FROM core_store WHERE is_active = 1 AND store_id > 0'
);

while ($store = $query->fetch(PDO::FETCH_OBJ)) {
    $installer->createUrlRewrite(
    	'channel/index/' . $store->store_id,
    	'channel.xml',
    	'channel/index',
    	$store->store_id
    );
