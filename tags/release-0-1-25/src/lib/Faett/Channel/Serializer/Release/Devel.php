...
/**
 * Faett_Channel_Serializer_Release_Devel
 *
 * NOTICE OF LICENSE
 * 
 ...
 */
class Faett_Channel_Serializer_Release_Devel
    ...
    public function serialize()
    {
        try {
            ...
            $version = '';
            // load the beta release's for the requested package
            $collection = Mage::getResourceModel('package/link_collection')
                ->addFieldToFilter('product_id', $this->_package->getId())
                ->addFieldToFilter('state', 'devel')
                ->addOrder('version', 'ASC');
            // return the latest development release version
