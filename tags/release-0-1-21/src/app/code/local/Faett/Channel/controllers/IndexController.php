...
class Faett_Channel_IndexController
    ...
    public function preDispatch()
    {
        try {
			...
			$this->_internalServerError($e->getMessage());
		}
    }
    ...
    protected function _authorize(array $args = array()) {
        // the API path for loading the ACL's for
        $apiPath = $this->_params[Faett_Channel_Helper_Data::TYPE];
        ...
        if (empty($resourceName) || empty($methodName)) {
            throw Faett_Channel_Exception_NoResourcePathException::create(
        ...
        if (!isset($resources->$resourceName)
            ...
            throw Faett_Channel_Exception_InvalidResourcePathException::create(
	...
	public function indexAction()
	{
        ...
        $this->_sendUploadResponse(
            Mage::getModel('channel/channel')->__toXML()
        );
	}
	...
	public function cAction()
	{
		try {
            ...
            $this->_sendUploadResponse(
                $category->__toXML(),
                $this->_params[Faett_Channel_Helper_Data::CONTENT_TYPE]
            );
	 ...
	 *
	 * @return string The server signature $_SERVER['SERVER_SIGNATURE]
	 */
	protected function _getServerSignature()
	{
	    return $this->getRequest()->getServer('SERVER_SIGNATURE');
	}
	 ...
	 * @return void
	 */
	protected function _internalServerError($content)
	{
	 ...
	 * @return void
	 */
	protected function _notFound($content)
	{
	 ...
	 *
	 * @param string $content The message if the user is not authenticated
	 * @return void
	 */
	protected function _unauthorized($content)
	{
	    // load the channel name
	    $channelName = Mage::helper('channel')->getChannelName();
        // and request authentication
	    header('WWW-Authenticate: Basic realm="' . $channelName . '"');
	 ...
	 * @return void
	 */
	protected function _forbidden($content)
	{
	...
	/**
	 * Adds the passed content to the response.
	 *
	 * @param string $content The content to add to the response
